...
function getCacheScope() {
	const scope = globalThis as typeof globalThis & {
		...
		__afterwordAstroLatestTrackCache?: { expiresAt: number; item: MediaTimelineItem | null } | null;
		...
		__afterwordAlbumWhaleLinkMap?: Promise<Map<string, { label: string; url: string }[]>>;
...
function extractMarkdownLink(body: string, pattern: RegExp) {
	const match = String(body || "").match(new RegExp(`\\[([^\\]]+)\\]\\(([^)]+)\\)`, "gi"));
...
async function getAlbumWhalePageMap() {
	...
	if (!scope.__afterwordAlbumWhalePageMap) {
		scope.__afterwordAlbumWhalePageMap = (async () => {
			try {
				const response = await fetchWithTimeout(ALBUMWHALE_LIST_URL, {
					headers: { "User-Agent": "afterword-astro media sync" },
				}, MEDIA_FETCH_TIMEOUT_MS);
				if (!response.ok) return new Map<string, string>();

				const html = await response.text();
				const items = [...html.matchAll(/<li id="(album_\d+)"[\s\S]*?<a[^>]+href="([^"]+)"[^>]*>/gi)];
				return new Map(items.map((match) => [match[1], new URL(match[2], ALBUMWHALE_LIST_URL).toString()]));
			} catch {
...
async function fetchRemoteAlbums(): Promise<AlbumEntry[]> {
	...
	try {
		const response = await fetchWithTimeout(ALBUMWHALE_FEED_URL, {
			headers: { "User-Agent": "afterword-astro media sync" },
		}, MEDIA_FETCH_TIMEOUT_MS);
		...
		return entries
			.map((match) => {
				const entry = match[1] || "";
				const title = decodeHtmlEntities(entry.match(/<title>([\s\S]*?)<\/title>/i)?.[1] || "Untitled").trim();
				const sourceUrl = decodeHtmlEntities(
					entry.match(/<link[^>]+rel="alternate"[^>]+href="([^"]+)"/i)?.[1] || "",
				).trim();
				const publishedAt = new Date(
					decodeHtmlEntities(entry.match(/<published>([\s\S]*?)<\/published>/i)?.[1] || String(Date.now())),
				);
...
async function fetchRemoteTracks(): Promise<TrackEntry[]> {
	try {
		const response = await fetchWithTimeout(CRUCIAL_TRACKS_FEED_URL, {
			headers: { "User-Agent": "afterword-astro media sync" },
		}, MEDIA_FETCH_TIMEOUT_MS);
		if (!response.ok) return [];
		...
		return (data.items || [])
			.map((item) => {
				...
				const noteHtml = extractTrackNoteHtmlFromContentHtml(String(item.content_html || ""));
				const slug = createTrackSlugFromPublishedAt(publishedAt, trackTitle, artist);
				const listenLinks: { label: string; url: string }[] = [];
				if (sourceUrl) listenLinks.push({ label: "Crucial Tracks", url: sourceUrl });
				if (item._song_details?.apple_music_url) {
					listenLinks.push({ label: "Apple Music", url: item._song_details.apple_music_url });
				}
				if (item._song_details?.songlink_url) {
					listenLinks.push({ label: "Listen elsewhere", url: item._song_details.songlink_url });
				}
			...
			.filter((item) => item.trackTitle && !Number.isNaN(item.publishedAt.getTime()))
			.sort((left, right) => right.publishedAt.getTime() - left.publishedAt.getTime());
	} catch {
		return [];
	}
...
async function resolveAtprotoService(runtimeEnv: RuntimeEnv, identifier: string) {
	const did = await resolveAtprotoDid(identifier);
	const configured = normalizeString(runtimeEnv.STANDARD_SITE_PDS_URL || runtimeEnv.ATPROTO_PDS_URL).replace(/\/+$/, "");
	if (configured) {
	...
	const response = await fetchWithTimeout(`https://plc.directory/${encodeURIComponent(did)}`, {}, MEDIA_FETCH_TIMEOUT_MS);
	if (!response.ok) {
...
function getRepo(runtimeEnv: RuntimeEnv) {
	return (
		normalizeString(runtimeEnv.ATPROTO_REPO || runtimeEnv.STANDARD_SITE_IDENTIFIER || runtimeEnv.ATPROTO_IDENTIFIER) ||
		DEFAULT_REPO
	);
}
...
function normalizePopfeedItem(
	...
	did: string,
	serviceUrl: string,
	listsByUri: Map<string, { name: string; description: string; listType: string }>,
): PopfeedItem | null {
	const value = (record.value as Record<string, unknown>) || {};
	...
	const id = String(record.uri || "").split("/").pop() || "";
	...
	const posterImage =
		(posterRef ? getBlobUrl(serviceUrl, did, posterRef) : null) || normalizeString(value.posterUrl) || null;
...
async function fetchPopfeedItems(): Promise<PopfeedItem[]> {
	try {
		...
		return itemRecords
			.map((record) => normalizePopfeedItem(record, did, serviceUrl, listsByUri))
			.filter((item): item is PopfeedItem => Boolean(item))
			.filter((item) => item.listType !== "to_read_books")
			.sort((left, right) => right.date.getTime() - left.date.getTime() || left.title.localeCompare(right.title));
	} catch {
		return [];
	}
}

function toTimelineLinks(links: Array<{ label: string; url: string }>, limit = 3): MediaTimelineLink[] {
...
async function fetchMediaState() {
	...
	const request = Promise.all([fetchRemoteAlbums(), fetchRemoteTracks(), fetchPopfeedItems()])
	...
	try {
		...
		console.warn("[media] Returning empty media state after fetch failure:", error);
		return {
			expiresAt: Date.now() + MEDIA_CACHE_TTL_MS,
			albums: [],
			tracks: [],
...
export async function getLatestTrack() {
	...
	if (!scope.__afterwordAstroLatestTrackPromise) {
		scope.__afterwordAstroLatestTrackPromise = fetchRemoteTracks()
			.then((tracks) => {
...
export async function getMediaTimelinePage(offset = 0, limit = 24) {
	...
	const items = timeline.slice(normalizedOffset, normalizedOffset + normalizedLimit);
	const nextOffset =
		normalizedOffset + normalizedLimit < timeline.length ? normalizedOffset + normalizedLimit : null;
